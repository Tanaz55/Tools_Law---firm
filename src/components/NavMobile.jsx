        ...
        <nav className='lg:hidden relative z-20'>
            ...
            >
                ...
                <button onClick={() => setIsOpen(false)} className='absolute top-6 left-6'>
                    <img scr={CloseIcon} alt='' />
                </button>
                {navigation.map((item, index) => {
                    return (
                        <li key={index}>
                            <Link to={item.href} activeClass='active' spy={true} offset={-200} className='hover:text-accent-hover transition-all duration-300 cursor-pointer'>
                                {item.name}
                            </Link>
                        </li>
                    );
